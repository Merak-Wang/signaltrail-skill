...
**状态：** 实施中（Active） · **负责人：** 仓库维护者 · **最后验证：** 2026-09-23
...
该流程与常规日报及动态 HTML 新闻幻灯片分开。幻灯片使用已保存的日报/索引证据、有界讲解批次和确定性渲染，不依赖本文所述的实验审核回执。本文介绍的讲解/研究流程仍是实验性的快照工作流。`render --mode current` 会明确失败：自动更正链再取证、经过校准的逐主张
...
[并行研究工作流](research/2026-09-20-illustrated-workflow.md)新增日报前入口：冻结所选索引正文块，
检索、回答问题并编译研究底稿，再复用下述脚本与审核。`research bind` 连接实际完成日报，
`research render` 在原分析下追加图文，生成新的组合阅读版本。

单语输出用 `story --script SCRIPT.json --review RECEIPT.json` 接收该脚本的支持性审核。
可选 `beat.table` 将表头、单元格及编辑图注纳入完整审核；原创整理表没有来源原图注，
外部照片仍不准入。研究写作只读所需语言的风格卡，双语仍需双语一致性回执。
