...
See [the engineering principles](docs/design-docs/core-beliefs.md) for the decision rules behind
these boundaries.
...
total attempts, and gives the independent evaluator a minimal immutable report/index-hash dossier.
...
`reasoning_output` and `tool_call_output` are diagnostic subsets already included in output;
cache inclusion semantics are explicit. Missing host fields remain `unobservable` with `null`
values.
